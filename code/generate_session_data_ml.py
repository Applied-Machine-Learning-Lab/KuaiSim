...
if __name__ == '__main__':
    ...
    # torch.save(df,'df_ml')
    print('#user:', len(df['user_id'].unique()), ' #item:', len(df['movie_id'].unique()), ' #record:', len(df))
    ...
    with torch.no_grad():
        ...
        for i,uid in enumerate(users):
            ...
            user_dates = user_hist['timestamp'].unique()
            B = len(user_dates)
            ...
            row_ids = []
            for d in user_dates:
                day_session = user_hist[user_hist['timestamp'] == d]
            ...
            T = [dt.strptime(str(d)[:10], "%Y-%m-%d") for d in user_dates]
